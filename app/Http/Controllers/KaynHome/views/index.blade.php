@extends('frontend')

@section('CONTENT_REGION')
        <myheader></myheader>
        <main>
            <div class="container-fluid">
                <!-- banner top -->
                <section class="banner-home d-md-flex">
                    <div class="banner-center col p-0">
                        <div class="banner-item">
                            <a href="#">
                                <img src="images/banner-big-home.png" alt="">
                            </a>
                        </div>
                    </div>
                    <div class="banner-right d-flex flex-column flex-sm-row flex-md-column">
                        <div class="banner-item">
                            <a href="#">
                                <img src="images/banner-right-1.png" alt="">
                            </a>
                        </div>
                        <div class="banner-item">
                            <a href="#">
                                <img src="images/banner-right-2.png" alt="">
                            </a>
                        </div>
                        <div class="banner-item">
                            <a href="#">
                                <img src="images/banner-right-3.png" alt="">
                            </a>
                        </div>
                    </div>
                </section>
                <!-- end banner top -->

                <!-- banner adv -->
                <section class="banner-content text-center">
                    <a href="#">
                        <img src="images/banner-phien-cho-buoi-sang.png" alt="">
                    </a>
                </section>
                <!-- end banner adv -->
                
                <!-- product sale -->
                <section class="section-product-sale">
                    <div class="sale-title d-flex flex-wrap align-items-center">
                        <h4>BIG SALE</h4>
                        <div class="d-flex align-items-center col p-0">
                            <div class="time-down text-nowrap">
                                <span>05</span>
                                <i>:</i>
                                <span>00</span>
                                <i>:</i>
                                <span>00</span>
                            </div>
                            <a href="#" class="view-all ml-auto">
                                Xem tất cả >>
                            </a>
                        </div>
                    </div>
                    <div class="wrap-product-slider">
                        <div class="product-sale-slider owl-carousel owl-theme">
                            <div class="item">
                                <product-sale></product-sale>
                            </div>
                            <div class="item">
                                <product-sale></product-sale>
                            </div>
                            <div class="item">
                                <product-sale></product-sale>
                            </div>
                            <div class="item">
                                <product-sale></product-sale>
                            </div>
                            <div class="item">
                                <product-sale></product-sale>
                            </div>
                            <div class="item">
                                <product-sale></product-sale>
                            </div>
                            <div class="item">
                                <product-sale></product-sale>
                            </div>
                            <div class="item">
                                <product-sale></product-sale>
                            </div>
                            <div class="item">
                                <product-sale></product-sale>
                            </div>
                        </div>
                    </div>
                </section>
                <!-- end product sale -->


                <!-- product sugget -->
                <section class="section-product-sugget">
                    <div class="site-title d-flex flex-wrap align-items-center">
                        <a href="#">
                            <h4>
                                <i class="icon icon-star"></i>
                                GỢI Ý DÀNH RIÊNG CHO BẠN
                            </h4>
                        </a>
                        <div class="d-flex align-items-center col px-0">
                            <a href="#" class="view-all ml-auto">
                                Xem tất cả >>
                            </a>
                        </div>
                    </div>
                    <div class="wrap-sugget">
                        <div class="sugget-product-slider owl-carousel owl-theme">
                            <div class="item">
                                <product-sugget></product-sugget>
                            </div>
                            <div class="item">
                                <product-sugget></product-sugget>
                            </div>
                            <div class="item">
                                <product-sugget></product-sugget>
                            </div>
                            <div class="item">
                                <product-sugget></product-sugget>
                            </div>
                            <div class="item">
                                <product-sugget></product-sugget>
                            </div>
                            <div class="item">
                                <product-sugget></product-sugget>
                            </div>
                            <div class="item">
                                <product-sugget></product-sugget>
                            </div>
                            <div class="item">
                                <product-sugget></product-sugget>
                            </div>
                            <div class="item">
                                <product-sugget></product-sugget>
                            </div>
                            <div class="item">
                                <product-sugget></product-sugget>
                            </div>
                        </div>
                    </div>
                </section>
                <!-- end product sugget -->

                <!-- product feature -->
                <section class="section-product-feature">
                    <div class="site-title d-flex flex-wrap align-items-center">
                        <a href="#">
                            <h4>
                                <i class="icon icon-star"></i>
                                SẢN PHẨM NỔI BẬT
                            </h4>
                        </a>
                        <div class="d-flex align-items-center col px-0">
                            <a href="#" class="view-all ml-auto">
                                Xem tất cả >>
                            </a>
                        </div>
                    </div>
                    <div class="product-list">
                        <div class="row">
                            <div class="col-6 col-sm-4 col-lg-3 col-xl-25">
                                <product-item></product-item>
                            </div>
                            <div class="col-6 col-sm-4 col-lg-3 col-xl-25">
                                <product-item></product-item>
                            </div>
                            <div class="col-6 col-sm-4 col-lg-3 col-xl-25">
                                <product-item></product-item>
                            </div>
                            <div class="col-6 col-sm-4 col-lg-3 col-xl-25">
                                <product-item></product-item>
                            </div>
                            <div class="col-6 col-sm-4 col-lg-3 col-xl-25">
                                <product-item></product-item>
                            </div>
                            <div class="col-6 col-sm-4 col-lg-3 col-xl-25">
                                <product-item></product-item>
                            </div>
                            <div class="col-6 col-sm-4 col-lg-3 col-xl-25">
                                <product-item></product-item>
                            </div>
                            <div class="col-6 col-sm-4 col-lg-3 col-xl-25">
                                <product-item></product-item>
                            </div>
                            <div class="col-6 col-sm-4 col-lg-3 col-xl-25">
                                <product-item></product-item>
                            </div>
                            <div class="col-6 col-sm-4 col-lg-3 col-xl-25">
                                <product-item></product-item>
                            </div>
                        </div>
                    </div>
                </section>
                <!-- end product feature -->
                
                <!-- product list home -->
                <section class="product-list-home">
                    <div class="site-title d-flex flex-wrap align-items-center">
                        <a href="#">
                            <h4>
                                <i class="icon icon-star"></i>
                                HÀNG TIÊU DÙNG
                            </h4>
                        </a>
                        <div class="d-flex align-items-center col px-0">
                            <ul class="list-cat d-flex">
                                <li>
                                    <a href="#">CHĂM SÓC DA MẶT</a>
                                </li>
                                <li>
                                    <a href="#">CHĂM SÓC TÓC</a>
                                </li>
                                <li>
                                    <a href="#">CHĂM SÓC DA</a>
                                </li>
                                <li>
                                    <a href="#">CHĂM SÓC BODY</a>
                                </li>
                            </ul>
                            <a href="#" class="view-all ml-auto">
                                Xem tất cả >>
                            </a>
                        </div>
                    </div>
                    
                    <div class="product-list">
                        <div class="row">
                            <div class="col-xl-25">
                                <a href="#" class="banner-product-home">
                                    <img src="images/banner-product-home-1.png" class="w-100" alt="">
                                </a>
                            </div>
                            <div class="col">
                                <div class="row">
                                    <div class="col-6 col-sm-4 col-lg-3">
                                        <product-item></product-item>
                                    </div>
                                    <div class="col-6 col-sm-4 col-lg-3">
                                        <product-item></product-item>
                                    </div>
                                    <div class="col-6 col-sm-4 col-lg-3">
                                        <product-item></product-item>
                                    </div>
                                    <div class="col-6 col-sm-4 col-lg-3">
                                        <product-item></product-item>
                                    </div>
                                    <div class="col-6 col-sm-4 col-lg-3">
                                        <product-item></product-item>
                                    </div>
                                    <div class="col-6 col-sm-4 col-lg-3">
                                        <product-item></product-item>
                                    </div>
                                    <div class="col-6 col-sm-4 col-lg-3">
                                        <product-item></product-item>
                                    </div>
                                    <div class="col-6 col-sm-4 col-lg-3">
                                        <product-item></product-item>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </section>
                <!-- end product list home -->

                <!-- product list home -->
                <section class="product-list-home">
                    <div class="site-title d-flex flex-wrap align-items-center">
                        <a href="#">
                            <h4>
                                <i class="icon icon-star"></i>
                                MỸ PHẨM
                            </h4>
                        </a>
                        <div class="d-flex align-items-center col px-0">
                            <ul class="list-cat d-flex">
                                <li>
                                    <a href="#">CHĂM SÓC DA MẶT</a>
                                </li>
                                <li>
                                    <a href="#">CHĂM SÓC TÓC</a>
                                </li>
                                <li>
                                    <a href="#">CHĂM SÓC DA</a>
                                </li>
                                <li>
                                    <a href="#">CHĂM SÓC BODY</a>
                                </li>
                            </ul>
                            <a href="#" class="view-all ml-auto">
                                Xem tất cả >>
                            </a>
                        </div>
                    </div>
                    
                    <div class="product-list">
                        <div class="row">
                            <div class="col-xl-25">
                                <a href="#" class="banner-product-home">
                                    <img src="images/banner-product-home-2.png" class="w-100" alt="">
                                </a>
                                <a href="#" class="banner-product-home">
                                    <img src="images/banner-product-home-2.png" class="w-100" alt="">
                                </a>
                            </div>
                            <div class="col">
                                <div class="row">
                                    <div class="col-6 col-sm-4 col-lg-3">
                                        <product-item></product-item>
                                    </div>
                                    <div class="col-6 col-sm-4 col-lg-3">
                                        <product-item></product-item>
                                    </div>
                                    <div class="col-6 col-sm-4 col-lg-3">
                                        <product-item></product-item>
                                    </div>
                                    <div class="col-6 col-sm-4 col-lg-3">
                                        <product-item></product-item>
                                    </div>
                                    <div class="col-6 col-sm-4 col-lg-3">
                                        <product-item></product-item>
                                    </div>
                                    <div class="col-6 col-sm-4 col-lg-3">
                                        <product-item></product-item>
                                    </div>
                                    <div class="col-6 col-sm-4 col-lg-3">
                                        <product-item></product-item>
                                    </div>
                                    <div class="col-6 col-sm-4 col-lg-3">
                                        <product-item></product-item>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </section>
                <!-- end product list home -->
                
                <section class="subscribe">
                    <div class="row align-items-end">
                        <div class="col-12 col-lg-5 d-flex align-items-lg-end subscribe-left flex-column align-items-center flex-lg-row">
                            <div class="image">
                                <img src="images/icon-email.png" alt="">
                            </div>
                            <div class="text-subscribe">
                                <h4>
                                    Hệ thống bán hàng
                                </h4>
                                <p>
                                    Đừng bỏ lỡ hàng ngàn sản phẩm và chương trình siêu hấp dẫn
                                </p>
                            </div>
                        </div>
                        <div class="col-12 col-lg-7 subscribe-right">
                            <h4>
                                ĐĂNG KÝ NHẬN TIN KHUYẾN MÃI
                            </h4>
                            <form action="" class="d-flex flex-column flex-sm-row">
                                <input type="text" placeholder="Nhập Email của bạn để đăng ký">
                                <button>ĐĂNG KÝ</button>
                            </form>
                        </div>
                    </div>
                </section>
            </div>
        </main>
        <myfooter></myfooter>

    <script src="js/vue.js"></script>
    <script src="js/httpVueLoader.js"></script>
    <script src="js/home.js"></script>

    <script src="js/jquery-3.4.1.js"></script>
    <script src="js/owl.carousel.min.js"></script>
    <script src="js/jquery.mmenu.all.js"></script>
    <script src="js/main.js"></script>
    <script>
        $(function() {
            $('.js-show-menu-active').addClass('isHome');
        });

    </script>
    <!-- Contact Area End -->

@stop